...
import { neckCheckboxes } from "../shared/constants";

export class NeckPage extends BasePage {
  // Selector
  private clickSanphamPage = "text=Sản Phẩm";
  readonly productItems = ".card";
  ...
  // 👉 Chọn các loại cổ áo (theo label)
  async selectCollars(...labels: string[]): Promise<void> {
    for (const label of labels) {
      const selector = neckCheckboxes[label];
      if (selector) {
  ...
  // 👉 Bỏ chọn các loại cổ áo (theo label)
